...
public class JWTAuthenticationMechanism implements HttpAuthenticationMechanism , Serializable {
   ...
   // private static final Logger LOGGER = Logger.getLogger(JWTAuthenticationMechanism.class.getName());
    ...
    public AuthenticationStatus validateRequest(HttpServletRequest request, HttpServletResponse response, HttpMessageContext context) {
       ...
       try{ 
        if((request.getSession().getAttribute("logged-group")==null) && (request.getRequestURI().contains("/AdminSite/") || request.getRequestURI().contains("/ClientSite/")))
            {
                System.out.println("In Phase Listener - blank group");
               request.getServletContext().getRequestDispatcher("/dashboard.jsf").forward(request, response);

            }
            else{
         		 if((request.getRequestURI().contains("/AdminSite/") && !request.getSession().getAttribute("logged-group").equals("Admin")))
          		{
         		 ...
         		 else if((request.getRequestURI().contains("/ClientSite/") && !request.getSession().getAttribute("logged-group").equals("User")))
         		 {
               // response.sendError(HttpServletResponse.SC_UNAUTHORIZED, "You are Not Authorised to view this Page . Go Back To Login");
                      request.getServletContext().getRequestDispatcher("/Unauthorised.jsf").forward(request, response);
